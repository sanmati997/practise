...
public class TrueForwardMain {
	...
	static List<Header> p_header_tab_i = new ArrayList<Header>();	
	static  List<Lines> p_lines_tab_i = new ArrayList<Lines>();
	...
	public static void main(String[] argv) {
		...
		trueFWDErrorTable = prop.getProperty("TrueFWDErrorTable");
		errorTableColsList = prop.getProperty("TrueFWDErrorTableColsList");
		...
		System.out.println("ERROR  Table : " + trueFWDErrorTable);
		...
		configProperties.put(SslConfigs.SSL_TRUSTSTORE_LOCATION_CONFIG, prop.getProperty("filepath_ts"));
		configProperties.put(SslConfigs.SSL_TRUSTSTORE_PASSWORD_CONFIG,  prop.getProperty("ssl_trust_pwd"));
		    // configure the following three settings for SSL Authentication
		configProperties.put(SslConfigs.SSL_KEYSTORE_LOCATION_CONFIG, prop.getProperty("filepath_ks"));
		configProperties.put(SslConfigs.SSL_KEYSTORE_PASSWORD_CONFIG, prop.getProperty("ssl_key_pwd"));
		configProperties.put(SslConfigs.SSL_KEY_PASSWORD_CONFIG, prop.getProperty("ssl_key_pwd"));
			...
			while (true) {
				...
				for (TopicPartition partition : records.partitions()) {
					...
					for (ConsumerRecord<String, String> record : partitionRecords) {
						...
						//if(JsongetString.contains("APPLICATION_NAME")||JsongetString.contains("CREATION_DATE")||JsongetString.contains("pos")) {
							//System.out.println("IGNORING WRONG JSON");
							//break;
						//}
						...
						TrueForwardCounsumerRespBean responseObj = null;                        
						try {
							objectMapper.disable(DeserializationFeature.ADJUST_DATES_TO_CONTEXT_TIME_ZONE);
							responseObj = objectMapper.readValue(JsongetString,TrueForwardCounsumerRespBean.class);
							...
							if (!Strings.isNullOrEmpty(JsongetString.toString())) {
								...
								if (arr1.length > 1) {
									...
									dbCon.insertErrorErrData("ID", Appl_Req_ID,topicName, intOffset,intPartition_Number, JsongetString,	err_code, message, "-1", "sysdate","-1", "sysdate", trueFWDErrorTable,errorTableColsList);
							...
							if (!Strings.isNullOrEmpty(JsongetString.toString())) {
								...
								System.out.println("Partition :"
										...
										+ " Offset going to updated :"
										+ intOffset);
								//hm.put(intPartition_Number, intOffset);//--double check
								//dbCon.updateCommitedOffset(hm, commitTable);
								continue;
							}
					...
					//TODO for now if condition is not required		
						//if (i % 5 == 0) { 
						...
						//	System.out.println("----- Firt batchsize : ");
							if (uniDataBatch.size() > 0
								...
								if (uniDataBatch.size() > 0) {
									...
									dbCon.executeTrueFWDSave(p_header_tab_i,p_lines_tab_i,topicName,intOffset,intPartition_Number,trueFWDErrorTable,errorTableColsList);
									p_header_tab_i.clear();
									p_lines_tab_i.clear();
								}
	...
	public static void prepareBatchQuery(commonUtility commObj,TrueForwardCounsumerRespBean acatCounsumerRespBean, Integer offset, String JsongetString,int Partition_Number,  int counter) {

		try {
			System.out.println("********* Counter : " + counter);
														   
			p_header_tab_i.addAll(acatCounsumerRespBean.getheader());
			p_lines_tab_i.addAll(acatCounsumerRespBean.getlines());
